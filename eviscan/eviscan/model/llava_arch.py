...
from eviscan.eviscan.constants import IGNORE_INDEX, IMAGE_TOKEN_INDEX, DEFAULT_IMAGE_PATCH_TOKEN, DEFAULT_IM_START_TOKEN, DEFAULT_IM_END_TOKEN

from eviscan.eviscan.mm_utils import get_anyres_image_grid_shape
...
class LlavaMetaModel:
    ...
    def initialize_vision_modules(self, model_args, fsdp=None):
        ...
        self.sae=SiglipAE()
        self.sae.load_state_dict(torch.load('/share/LXRlxr0_0/code/eviscan/eviscan/eviscan/eviscan/model/encoder.pth'),strict=False)        
...
class LlavaMetaForCausalLM(ABC):
    ...
    def prepare_inputs_labels_for_multimodal(self, input_ids, position_ids, attention_mask, past_key_values, labels, images, modalities=["image"], image_sizes=None,time_embedding=None):
        ...
        if vision_tower is None or images is None or (input_ids.shape[1] == 1 and input_ids[0].item()!=-200):
        ...
        #video_token_indices=[]
        self.model.image_token_indices_in_visual = None
        for image_idx, image_feature in enumerate(image_features):
            if time_embedding[image_idx] is not None:
                mask = (time_embedding[image_idx] == 151654)
                indices = torch.nonzero(mask).squeeze()
                self.model.image_token_indices_in_visual = indices
